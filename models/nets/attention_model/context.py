...
class VRPContext(PrevNodeContext):
    ...
    def _state_embedding(self, embeddings, state):
        ...
        state_embedding = (state.VEHICLE_CAPACITY.unsqueeze(-1) - state.used_capacity.unsqueeze(-1))
        return state_embedding
    # def _state_embedding(self, embeddings, state):
    #     # embeddings: [batch, graph_size, embed_dim]
    #     # 从 state.states 中取出一维张量 capacity_limit 和 current_load，shape=[batch]
    #     capacity_limit = state.states["capacity_limit"]  # [batch]
    #     current_load   = state.states["current_load"]    # [batch]
    #     print(f"capacity_limit shape: {capacity_limit.shape}")
    #     print(f"current_load shape: {current_load.shape}")
    #     # 计算剩余载客量，得到 [batch]
    #     remaining_capacity = capacity_limit - current_load
    #     # 将其扩展为 [batch, 1, 1]，对应 PrevNodeContext.forward 中 prev_node_embedding [batch,1,embed_dim]
    #     state_embedding = remaining_capacity.unsqueeze(1)
    #     return state_embedding
