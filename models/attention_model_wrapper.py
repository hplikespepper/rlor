...
class Agent(nn.Module):
    ...
    def get_action_and_value_cached(self, x, action=None, state=None):
        if state is None:
            ...
            x = self.backbone.decode(x, state)
        logits = self.actor(x)
        # print(">>> logits entering Categorical <<<")
...
class stateWrapper:
    ...
    def __init__(self, states, device, problem="tsp"):
        ...
        if problem == "tsp":
            ...
            # 改为环境中的 capacity_limit，并直接用 current_load
            # 注意：capacity_limit 需要从 env 里传入 obs（可在 env._update_state 中新增一项）
            # 先在第 1 维和第 2 维各插入一个维度，得到形状 [batch, 1, 1]
            self.VEHICLE_CAPACITY = self.states["capacity_limit"]
            self.used_capacity   = self.states["current_load"]
