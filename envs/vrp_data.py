...
def make_instance(args):
    depot, loc, demand, capacity, *rest = args
    grid_size = 1
    # 生成 PD 数据：N 个取货 + N 个送货
    pickups = np.array(loc) / grid_size              # [N,2]
    dropoffs = np.random.rand(len(loc), 2)            # 随机送货点
    nodes = np.vstack((np.array(depot)/grid_size, pickups, dropoffs))
    # demand: 取货点 +1，送货点 -1
    pd_demand = np.concatenate((np.ones(len(loc)), -np.ones(len(loc))))
    return {
        "loc": nodes,          # shape=(2N+1,2)
        "demand": pd_demand,   # shape=(2N,)
        "depot": np.array(depot)/grid_size,
    }
